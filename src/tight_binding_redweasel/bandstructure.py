...
class BandStructureModel:
    ...
    def init_tight_binding(symmetry: Symmetry, neighbors, band_count, cos_reduced=False, exp=False) -> Self:
        ...
        f_i_tb, df_i_tb, ddf_i_tb, term_count, neighbors = _get_tight_binding_coeff_funcs(symmetry, neighbors, cos_reduced=cos_reduced, exp=exp)
    ...
    def init_tight_binding_from_ref(symmetry: Symmetry, neighbors, k_smpl, ref_bands, band_offset=0, add_bands=0, cos_reduced=False, exp=False) -> Self:
        f_i_tb, df_i_tb, ddf_i_tb, term_count, neighbors = _get_tight_binding_coeff_funcs(symmetry, neighbors, cos_reduced=cos_reduced, exp=exp)
        model = BandStructureModel.init_from_ref(f_i_tb, df_i_tb, ddf_i_tb, term_count, k_smpl, ref_bands, band_offset, add_bands)
        ...
        try:
            for iteration in range(iterations):
                ...
                if batch_div == 1:
                    ...
                    loss = np.linalg.norm(diff * band_weights) / len(batch)**.5 # this is how the loss is computed in self.error
                ...
                diff = eigvecs @ (diff[...,None] * np.conj(np.swapaxes(eigvecs, 1, 2)))
    ...
    def save(self, filename, format=None):
        ...
        np.set_printoptions(**opt) # reset printoptions to what they were before
...
def _get_tight_binding_coeff_funcs(sym, neighbors, cos_reduced=False, exp=False):
