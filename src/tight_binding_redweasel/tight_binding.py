...
class TightBindingModel:
    """
    This class implements a tight binding model with k-independent symmetry.  
    The functions in here are not well tested, so this class is not stable.
    """
    ...
    def __init__(self, unitary_repr: UnitaryRepresentation, neighbors, params):
        ...
        if params is not None:
            ...
            self.params = np.zeros((param_count,) + (np.shape(unitary_repr.U)[1],)*2, dtype=np.complex128)
    ...
    def init_from_ref(u_repr: UnitaryRepresentation, neighbors, k_smpl, ref_bands, use_repr_order=False, verbose=True):
        ...
        # TODO don't allow add_bands in the middle of the bandstructure!
    ...
    def f_i(self, k, i):
        ...
        s = 0.0
        scale = 2*np.pi
        part = i // len(self.neighbors)
        r_orig = self.neighbors[i % len(self.neighbors) + part]
