...
class AsymTightBindingModel:
    ...
    def __init__(self, neighbors, H_r):
        ...
        self.H_r = np.asarray(H_r).astype(np.complex128)
        pshape = np.shape(self.H_r)
        assert len(pshape) == 3
        assert pshape[1] == pshape[2]
        # checks, that neighbors and sym fits H_r
    ...
    def init_from_ref(neighbors, k_smpl, ref_bands):
        ...
        tb = AsymTightBindingModel(neighbors, H_r)
        tb.normalize()
        return tb

    def dim(self):
        return len(self.neighbors[0]) # dimension from neighbors
    ...
    def save(self, filename, format=None):
        ...
        if format == "json":
            ...
            json_tb_format.save(filename, self.neighbors, self.H_r_complex())
        ...
        if format == "python":
            with open(filename, "r") as file:
                ...
                if len(sym) > 1:
                    ...
                    model = AsymTightBindingModel(complete_neighbors, H_r[order])
    ...
    def optimize(self, k_smpl, k_smpl_weights, ref_bands, band_weights, band_offset, iterations, batch_div=1, learning_rate=1.0, train_k0=True, use_pinv=True, max_accel_global=None, regularization=1.0, keep_zeros=False, verbose=True):
        ...
        # mask for keep_zeros
        H_r_mask = np.where(np.abs(self.H_r) < 1e-14, 0.0, 1.0)
        # memoize self.f_i here using a rectangular matrix
        f_i = np.zeros((len(k_smpl), len(self.H_r)), dtype=np.complex128)
        ...
        last_add = np.zeros_like(self.H_r)
        self.normalize()
    ...
    def normalize(self):
        self.H_r[0] = (self.H_r[0] + self.H_r[0].T.conj()) / 2
        # normalize
        ...
        ev = ev.T[sorting].T
        for i in range(len(self.H_r)):
            self.H_r[i] = np.conj(ev.T) @ self.H_r[i] @ ev
        # TODO test the following
        # normalize a little more using complex reflections on the second matrix
        if len(self.H_r) > 1:
            for i in range(1, len(self.H_r[1])):
                x = self.H_r[1][i-1, i]
                a = np.abs(x)
                if a != 0:
                    sign = np.conj(x) / a
                    self.H_r[:, :, i] *= sign
                    self.H_r[:, i, :] *= np.conj(sign)
                else:
    ...
    def copy(self):
        return AsymTightBindingModel(self.neighbors.copy(), self.H_r.copy())
...
def test_asym_tight_binding_optimize():
    ...
    n = 5
    tb = AsymTightBindingModel(neighbors, np.zeros((len(neighbors), n, n)))
    ...
    for k_count in [1]:
        ...
        for kw in [1, 1.5]:
            for bw in [1, 1.5]:
                tb.set_from_complex(np.random.random(tb.H_r.shape))
